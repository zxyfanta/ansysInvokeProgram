...
class AircraftGenerator(BaseSimulator):
    ...
    def _export_to_stl(self, model_data: Dict[str, Any], output_path: Path):
        ...
        try:
            ...
            if len(triangles) == 0:
                self.log_error("没有三角形数据可导出")
                return

            # 写入STL文件（二进制格式）
            with open(output_path, 'wb') as f:
                # STL文件头（80字节）
                model_name = model_data.get('metadata', {}).get('name', 'Aircraft Model')
                header = f"Generated by Aircraft Modeling System - {model_name}"
                header = header[:79] + '\0'  # 确保以null结尾
                header = header.ljust(80, '\0')  # 填充到80字节
                ...
                for i, triangle in enumerate(triangles):
                    try:
                        ...
                        # 检查顶点有效性
                        if not (np.isfinite(v0).all() and np.isfinite(v1).all() and np.isfinite(v2).all()):
                            continue
                        ...
                        self.log_warning(f"跳过无效三角形 {i}: {e}")
                        continue
            ...
            self.log_info(f"三角形数量: {triangle_count}, 文件大小: {actual_size} 字节")
    ...
    def _generate_fuselage_mesh(self, fuselage_data: Dict[str, Any]) -> Tuple[List[List[float]], List[List[int]]]:
        ...
        for station in stations:
            x = station['x']
            radius = max(station['radius'], 0.001)  # 确保半径不为0
    ...
    def _generate_wing_mesh(self, wing_data: Dict[str, Any]) -> Tuple[List[List[float]], List[List[int]]]:
        ...
        # 检查并修复无效的弦长
        if chord_root <= 0:
            chord_root = span * 0.3  # 默认为展长的30%
        if chord_tip <= 0:
            chord_tip = chord_root * 0.6  # 默认为根弦长的60%
